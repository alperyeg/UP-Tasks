...
class Plotting:
    ...
    def plot_raster_bars(t_start, t_stop, n_rec, frac_to_plot, path, filename,
        ...
        for layer in Plotting.layers:
            spikes[layer] = {}
            for pop in Plotting.pops:
        ...
        rate_list = np.zeros(Plotting.n_layers*Plotting.n_pops_per_layer)
        for i, layer in enumerate(layer_list) :
            for j, pop in enumerate(pop_list) :
                rate_list[i*Plotting.n_pops_per_layer + j] = rates[layer][pop]
